...
# Add a variable to save a file to a path
...
# Read the csv and conver it into a list of dictionaries
with open(file_to_load, 'r') as election_data:
    ...
    for row in file_reader:
       ...
       # the candidate list
        ...
        # 4a: Write an if statement that checks the
...
with open(file_to_save, 'w') as election_analysis_file:
    ...
    for county_name in county_votes:
        ...
        votes_2 = county_votes[county_name]

        # 6c: Calculate the percentage of votes for the county.
        vote_percentage_2 = float(votes_2) / float(total_votes) * 100

        county_results = (f"{county_name}: {vote_percentage_2:.1f}% ({votes_2:,})\n")
        ...
        if (votes_2 > largest_county_count):

            # Create a summary for the largest county turnout
            # If the above is true, winning count = votes and winning county = county name
            largest_county_count = votes_2
    ...
    election_analysis_file.write(largest_county_summary)
